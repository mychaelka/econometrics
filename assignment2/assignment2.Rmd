...
discrimination but has no direct impact on both occupation and wages. Furthermore, we assume that discrimination influences both occupation and wages. Occupation also has a direct effect on wages. Also,
...
-- The open paths in this case are D -> O -> W and D -> W.  
...
-- The open paths in this case are D -> O -> W, D -> W and D <- G -> O -> W.  
...
**1. adjusting only for occupation:** Same as the previous case, but on top of that a new spurious path is open between gender and wages through ability. 
...
<center> $\huge \text{"Figures are great,}$
         $\huge \text{everyone should do that."}$ </center>
...
-- Given that by adjusting for occupation and ability, we are able to recover both the direct and indirect effect of discrimination on wages, we propose a linear model with OLS estimator, while controlling for occupation and ability, of the form
$$W = \beta_0 + \beta_1G + \beta_2 A + \beta_3 D + \varepsilon$$
...
**(1c) What is the true average treatment effect on the untreated (ATU) of job training programme on
earnings?**  
...
**(1d) What is the true average treatment effect on the treated of job training programme on earnings for
those from cities?**  
...
**(1e) What is the true average treatment effect on the treated of job training programme on earnings for
...
-- The effect is higher in absolute value than general ATT. 

**(1f) Suppose that you can persuade every person to either attend the training or not based on what is more
beneficial for her/him in terms of earnings. What would be the average effect of such ideal intervention?**  
```{r intervention}
dat_int <- dat
...
**(2a) Calculate unadjusted differences in mean observed outcomes job training participants (D = 1) and
...
$E[Y |D = 1] − E[Y |D = 0] = \overbrace{E[Y(1) | D = 1] − E[Y(0) | D = 1]} +$ 
$\underbrace{E[Y (0)|D = 1] − E[Y (0)|D = 0]}$  
...
<br/><br/>
<center> $\huge \text{"I am just masking my incompetence}$
         $\huge \text{by making it colorful."}$ </center>
...
The IV regression estimate yields a coefficient of 0.094, while the coefficient reported in Angrist and Krueger is 0.089. The standard errors are very similar. The OLS estimate is also similar to the authors' results and to the IV estimate.
Again, only one part of one table is replicated due to the lack of data.
...
<center> $\huge \text{"You should be laughing by now,}$
         $\huge \text{but I understand that you're tired."}$  
